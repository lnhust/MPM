...
_MINI_IMAGENET_DATASET_DIR = '/home/ln/Data/MiniImagenet'
IMAGE_SIZE = 84
...
class TieredImageNet(data.Dataset):
    def __init__(self, phase='train', do_not_use_random_transf=False):
        ...
        self.base_folder = 'tieredImagenet'
        assert(phase=='train' or phase=='val' or phase=='test')
        self.phase = phase
        ...
        file_train_categories_train_phase = os.path.join(
            _TIERED_IMAGENET_DATASET_DIR, 'splits',
            'train_list_with_label.txt')
        file_train_categories_val_phase = os.path.join(
            _TIERED_IMAGENET_DATASET_DIR, 'splits',
            'train_list_with_label.txt')
        file_train_categories_test_phase = os.path.join(
        ...
        file_val_categories_val_phase = os.path.join(
            _TIERED_IMAGENET_DATASET_DIR, 'splits',
            'val_list_with_label.txt')
        file_test_categories_test_phase = os.path.join(
            _TIERED_IMAGENET_DATASET_DIR, 'splits',
            'test_list_with_label.txt')

        if self.phase=='train':
            # During training phase we only load the training phase images
            # of the training categories (aka base categories).
            data_train = load_data_list(file_train_categories_train_phase)
            self.data = data_train['data']
            self.labels = data_train['labels']
            ...
            if self.phase=='test':
                # load data that will be used for evaluating the recognition
                # accuracy of the base categories.
                data_base = load_data_list(file_train_categories_test_phase)
                # load data that will be use for evaluating the few-shot recogniton
                # accuracy on the novel categories.
                data_novel = load_data_list(file_test_categories_test_phase)
            else: # phase=='val'
                # load data that will be used for evaluating the recognition
                # accuracy of the base categories.
                data_base = load_data_list(file_train_categories_val_phase)
                # load data that will be use for evaluating the few-shot recogniton
                # accuracy on the novel categories.
                data_novel = load_data_list(file_val_categories_val_phase)

            self.data = data_base['data'] + data_novel['data']
            self.labels = data_base['labels'] + data_novel['labels']
    ...
    def __getitem__(self, index):
        impath, label = self.data[index], self.labels[index]
        # to return a PIL Image
...
class FewShotDataloader():
    def __init__(self,
        ...
        self.nTestBase = nTestBase if self.nKbase > 0 else 0
    ...
    def sample_train_and_test_examples_for_novel_categories(
        ...
        for Knovel_idx in range(len(Knovel)):
            ...
            Exemplars.append(imds_ememplars)
            Idxes.append(imd_ids)

        return Tnovel, Exemplars, Idxes
    ...
    def sample_episode(self):
        ...
        Tnovel, Exemplars, Idxes = self.sample_train_and_test_examples_for_novel_categories(
            Knovel, nTestNovel, nExemplars, nKbase)

        # concatenate the base and novel category examples.
        Test = Tbase + Tnovel

        return Exemplars, Test, Kbase, Knovel, Idxes
    ...
    def createExemplarsTensorData(self, examples):
        """
        Creates the examples image and label tensor data.

        Args:
            examples: a list in which each item representing a
                list of exemplars for one category.

        Returns:
            images: a tensor of shape [nClass, nExempler, Height, Width, 3] with the
                example images.
        """
        images = []
        for i in range(self.nKnovel):
            images_per_cat = torch.stack(
                [self.dataset[img_idx][0] for img_idx in examples[i]], dim=0)
            images.append(images_per_cat)
    ...
    def get_iterator(self, epoch=0):
        ...
        def load_function(iter_idx):
            Exemplars, Test, Kbase, Knovel, Idxes = self.sample_episode()
            Kbase = torch.LongTensor(Kbase) if len(Kbase) else torch.LongTensor([-1])
            Knovel = torch.LongTensor(Knovel)
            Idxes = torch.LongTensor(Idxes)
            Xt, Yt = self.createQueryTensorData(Test)
            if len(Exemplars) > 0:
                Xe = self.createExemplarsTensorData(Exemplars)
                return Xe, Xt, Yt, Kbase, Knovel, Idxes
            else:
                return Xt, Yt, Kbase
...
class TaskSampler(data.Sampler):
    def __init__(self, file_path):
        ...
        idxes = []
        for line in lines:
            idxes.append([int(res) for res in line.strip().split(',')])
        self.idxes = idxes
